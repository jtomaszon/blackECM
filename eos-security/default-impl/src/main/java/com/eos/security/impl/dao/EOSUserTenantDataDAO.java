...
public class EOSUserTenantDataDAO extends AbstractDAO<EOSUserTenantDataEntity> {
	...
	public void updateUserData(String login, String key, String value,
			Long tenantId) {
	...
	public String findUserData(String login, String key, Long tenantId) {
		try {
			return em
					.createNamedQuery(EOSUserTenantDataEntity.QUERY_FIND,
							String.class)
					.setParameter(EOSUserTenantDataEntity.PARAM_LOGIN, login)
					.setParameter(EOSUserTenantDataEntity.PARAM_KEY, key)
					.setParameter(EOSUserTenantDataEntity.PARAM_TENANT,
							tenantId).getSingleResult();
		} catch (PersistenceException e) {
			return null;
		}
	}

	public List<EOSUserTenantDataEntity> listUserData(String login, int limit,
			int offset, Long tenantId) {
		return em
				.createNamedQuery(EOSUserTenantDataEntity.QUERY_LIST,
						EOSUserTenantDataEntity.class)
				.setParameter(EOSUserTenantDataEntity.PARAM_LOGIN, login)
				.setParameter(EOSUserTenantDataEntity.PARAM_TENANT, tenantId)
				.setFirstResult(offset).setMaxResults(limit).getResultList();
	}

	public List<EOSUserTenantDataEntity> listUserData(String login,
			List<String> keys, Long tenantId) {
		return em
				.createNamedQuery(EOSUserTenantDataEntity.QUERY_BY_KEY,
						EOSUserTenantDataEntity.class)
				.setParameter(EOSUserTenantDataEntity.PARAM_LOGIN, login)
				.setParameter(EOSUserTenantDataEntity.PARAM_KEY, keys)
				.setParameter(EOSUserTenantDataEntity.PARAM_TENANT, tenantId)
